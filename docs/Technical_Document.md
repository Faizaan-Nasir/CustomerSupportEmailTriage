...
This section defines the required Row Level Security (RLS) policies for the Supabase storage bucket used to store customer attachments such as invoices, images, and documents.

The bucket must be configured as **private**. All access must be mediated through the backend using the service role key and signed URLs.
...
##### 2. Insert Policy (Upload via Backend Only)

Purpose:
Allow backend services (using service role key) to upload files to the `attachments` bucket.

	create policy "Allow backend upload to attachments"
	on storage.objects
	for insert
	with check (
...
##### 3. Select Policy (Deny Direct Public Access)

Purpose:
Prevent direct file access from clients. All file access must happen via signed URLs generated by the backend.

	create policy "Deny public read access to attachments"
	on storage.objects
	for select
	using (
		false
...
##### 4. Delete Policy (Backend Cleanup)

Purpose:
Allow backend to delete files when tickets or attachments are removed.

	create policy "Allow backend delete from attachments"
	on storage.objects
	for delete
	using (
		bucket_id = 'attachments'
...
##### 5. (Optional) Path-Based Restriction (Recommended for Future)

If files are stored using ticket-based paths:

Example path:
	attachments/{ticket_id}/file.pdf

You can enforce stricter control:

	create policy "Scoped access by bucket only"
	on storage.objects
	for all
	using (
		bucket_id = 'attachments'
...
##### 6. Access Pattern (Important)

Files should never be accessed directly via public URLs.

Correct flow:

1. Backend uploads file to bucket
2. Backend stores file path in database
3. Frontend requests file access from backend
4. Backend generates signed URL:

	supabase.storage
		.from("attachments")
		.create_signed_url(path, expires_in)

5. Frontend uses signed URL to fetch file
...
- Do NOT expose raw file paths to frontend
- Always use short-lived signed URLs (e.g., 60–300 seconds)
- Always perform uploads via backend only

---

##### 8. Summary

This policy setup ensures:

- Secure storage of sensitive customer data
- Controlled access via backend
- Compatibility with RAG pipeline and attachment processing
...
Option B: SMTP
    ...
    GMAIL_SMTP_PASSWORD=your-smtp-password 

Purpose:
- Send outbound emails

For demonstration purposes, we will be using option B.
Incoming emails will not be read - instead they will be fed in as user inputs in front-end.
---
